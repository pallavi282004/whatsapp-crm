...
# Define router instance before usage
...
async def generate_message(
    ...
    # Stub: Replace with real multilingual message generation logic
    # For demo, just echo the prompt and language
    # Always return multilingual Diwali greetings regardless of prompt
    message = (
        'happy diwali"\n'
        'దీపావళి శుభాకాంక్షలు\n'
        'शुभ दीपावली \n'
        '¡Felices fiestas de Diwali!'
    )
...
# --- Signup Endpoint ---
@router.post("/signup/")
async def signup(request: Request, db: AsyncSession = Depends(database.get_db)):
    data = await request.json()
    username = data.get("username")
    email = data.get("email")
    password = data.get("password")
    wabaid = data.get("WABAID")
    p_access_token = data.get("PAccessToken")
    phone_id = data.get("Phone_id")
    if not all([username, email, password, wabaid, p_access_token, phone_id]):
...
async def login(request: Request, db: AsyncSession = Depends(database.get_db)):
    ...
    payload = {
        "sub": user.id if hasattr(user, 'id') else 1,
        "email": email,
